document.addEventListener('DOMContentLoaded', () => {
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            buttonText.textContent = currentView === 'create' ? 'Create' : 'Join';
            actionButton.className = `action-button ${currentView}`;
        });
    });

    removeButton.addEventListener('click', () => {
        roomCodeInput.value = ''; roomPasswordInput.value = '';
    ...
    roomForm.addEventListener('submit', (e) => {
        ...
        if (roomForm.code.value.length < 4) {
            codeError.textContent = 'Code must be at least 4 characters.'; 
            isValid = false;
        } else if (!/^[A-Za-z0-9]+$/.test(roomForm.code.value)) {
            codeError.textContent = 'Code can only contain letters and numbers.'; 
            isValid = false;
        }
        
        if (roomForm.password.value.length < 5) {
            passwordError.textContent = 'Password must be at least 5 numbers.'; 
            isValid = false;
        } else if (!/^[0-9]+$/.test(roomForm.password.value)) {
