...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', ({ code, password }) => {
    ...
    rooms[code] = { password: password, users: [], messages: [] };
  ...
  socket.on('joinRoom', ({ code, password }) => {
    ...
    if (!room) { return socket.emit('roomError', 'No room found with that code.'); }
    if (room.password !== password) { return socket.emit('roomError', 'Incorrect password.'); }
    if (room.users.length >= 2) { return socket.emit('roomError', 'This room is full.'); }
    socket.emit('roomActionSuccess');
  });
  ...
  socket.on('enterRoom', ({ code, user }) => {
    ...
    socket.emit('initialRoomData', { messages: room.messages, peer: peer });
    if (peer) {
  ...
  socket.on('typing', ({ roomCode }) => { socket.to(roomCode).emit('peerTyping'); });
  socket.on('stopTyping', ({ roomCode }) => { socket.to(roomCode).emit('peerStoppedTyping'); });
